...
int process_cli_requests(int svr_socket){
    ...
    while(1){
        ...
        if ((cli_socket = accept(svr_socket, NULL, NULL)) < 0) {
        ...
        if (rc == OK_EXIT) {
            // If client sends "exit", close this connection and continue waiting
            close(cli_socket);
            continue;
        } else if (rc == -1) {
            // If client requests to stop the server
            close(cli_socket);
            break;
        }
        close(cli_socket);
...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        // Check if single command is internal
        Built_In_Cmds cmd_type = rsh_match_command(cmd_list.commands[0].argv[0]);
        if (cmd_list.num == 1 && cmd_type != BI_NOT_BI) {
            if(cmd_type == BI_CMD_EXIT) {
                // If result is 5, then command is exit
                free(io_buff);
                return OK_EXIT;
            } else if (cmd_type == BI_CMD_STOP_SVR) {
                // If result is 4, then command is stop-server
                free(io_buff);
                return -1;
            } else if (cmd_type == BI_CMD_CD) {
                // If result is 2, then command is cd
                if (cmd_list.commands[0].argc > 1) {
                    if (chdir(cmd_list.commands[0].argv[1]) != 0) {
                        ...
                        //  - etc.
                        send_message_string(cli_socket, "cd: directory change failed\n");
                    }
                } else {
                    // cd with no args - go to home directory
                    char *home = getenv("HOME");
                    if (home && chdir(home) != 0) {
                        send_message_string(cli_socket, "cd: home directory change failed\n");
                    }
                }
        ...
        // After executing the command (whether built-in or external), send EOF
        send_message_eof(cli_socket);
    }
...
int send_message_string(int cli_socket, char *buff){
    ...
    rc = send(cli_socket, buff, strlen(buff), 0);
    if (rc < 0) {
        return ERR_RDSH_COMMUNICATION;
    }

    // Send EOF character to client when command execution is completed
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        {
            // Child process
            // If first command in pipeline, get input from client socket
            if (i == 0)
            {
                if (dup2(cli_sock, STDIN_FILENO) == -1)
                {
                    perror("dup2 client sock to stdin");
                    exit(ERR_EXEC_CMD);
                }
            }
            else
            ...
            // If last command is in the pipeline, send output to client socket
            if (i == clist->num - 1)
            {
                if (dup2(cli_sock, STDOUT_FILENO) == -1)
                {
                    perror("dup2 client sock to stdout");
                    exit(ERR_EXEC_CMD);
                }
                if (dup2(cli_sock, STDERR_FILENO) == -1)
                {
                    perror("dup2 client sock to stderr");
                    exit(ERR_EXEC_CMD);
                }
            }
            else
            {
                // If this is not the last command, set up stdout to next pipe
            ...
            // Close client socket as it's been duplicated where needed
            close(cli_sock);
            ...
            // Parent process
            // Only close pipe ends that are no longer needed anymore
            if (i > 0) {
 ...
 * 
 *  Again, using this function is entirely optional if you have or want to handle built-in
 *  commands.  
 * 
 *  Returns:
 * 
