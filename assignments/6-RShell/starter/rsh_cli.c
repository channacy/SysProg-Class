...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    // Allocate memory for cmd buff
    cmd_buff = (char *)malloc(SH_CMD_MAX * sizeof(char));
    if (cmd_buff == NULL) {
        perror("malloc failed");
        return ERR_MEMORY;
    }
    // Allocate memory for response buff
    ...
    while (1) 
    {
        ...
        /* Remove the trailing newline (if any) so that "ls" is sent instead of "ls\n" */
        cmd_buff[strcspn(cmd_buff, "\n")] = '\0';
        
        // TODO send() over cli_socket
        int send_len = strlen(cmd_buff) + 1;    //the +1 includes the NULL byte
        int bytes_sent;
        ...
        while ((io_size = recv(cli_socket, rsp_buff, RDSH_COMM_BUFF_SZ, 0)) > 0){
            if (io_size < 0) {
                // we got an error, handle it and break out of loop or return from function
                perror("recv error");
                return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_COMMUNICATION);
            }
            if (io_size == 0) {
            ...
            // Check if last byte is EOF
            is_eof = ((char)rsp_buff[io_size-1] == eof_char) ? 1 : 0;
            // Print the response (excluding EOF if present)
            if (is_eof) {
            ...
            // If EOF received, break to get next command
            if (is_eof) {
                break;
            }
        }
